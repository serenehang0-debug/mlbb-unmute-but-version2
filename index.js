  ...
  try {
    ...
    if (config.EXEMPT_CHANNELS.includes(newChannel.id)) {
      ...
      if (activeMuteTimers.has(member.id)) {
        clearTimeout(activeMuteTimers.get(member.id));
        activeMuteTimers.delete(member.id);
        logger.debug("Cleared timer for exempt channel", { user: member.user.tag });
      }
      return;
    }

    // Handle channel switches - clear existing timer if switching channels
    if (oldChannel && newChannel && oldChannel.id !== newChannel.id) {
      const wasOldExempt = config.EXEMPT_CHANNELS.includes(oldChannel.id);
      const isNewExempt = config.EXEMPT_CHANNELS.includes(newChannel.id);
      
      logger.debug("Channel switch detected", {
        user: member.user.tag,
        fromChannel: oldChannel.name,
        toChannel: newChannel.name,
        wasOldExempt,
        isNewExempt,
        wasMuted: member.voice.selfMute || member.voice.selfDeaf
      });
      
      if (activeMuteTimers.has(member.id)) {
        clearTimeout(activeMuteTimers.get(member.id));
        activeMuteTimers.delete(member.id);
        logger.debug("Cleared timer due to channel switch", { 
          user: member.user.tag,
          fromChannel: oldChannel.name,
          toChannel: newChannel.name
        });
    ...
    // Handle muted join or channel switch while muted
    if (member.voice.selfMute || member.voice.selfDeaf) {
      // Check if user is switching channels while muted
      const isChannelSwitch = oldChannel && newChannel && oldChannel.id !== newChannel.id;
      
      // Allow new timer if switching channels or no existing timer
      if (!isChannelSwitch && activeMuteTimers.has(member.id)) {
        logger.debug("Timer already exists for user", { user: member.user.tag });
        return;
      }
      ...
      // Batch logs
      const action = isChannelSwitch ? "switched to" : "joined";
      logQueue.push(
        `⚠️ **${member.user.tag}** ${action} **${newChannel.name}** muted/deafened.`
      );
      ...
      try {
        const message = isChannelSwitch 
          ? `🏆 **Hey ${member.displayName}!** You switched to **${newChannel.name}** while muted/deafened.\n⏱️ You have **${config.TIMEOUT / 1000}s** to unmute or you'll be disconnected. ⚔️`
          : `🏆 **Hey ${member.displayName}!** You're muted/deafened in **${newChannel.name}**.\n⏱️ You have **${config.TIMEOUT / 1000}s** to unmute or you'll be disconnected. ⚔️`;
        
        await member.send(message);
        logger.debug("Sent warning DM to user", { 
          user: member.user.tag, 
          channel: newChannel.name,
          action: isChannelSwitch ? "channel switch" : "join"
        });
      } catch (err) {
