...
class Config {
  ...
  validateChannels(guild) {
    ...
    // Validate exempt channels
    const missingExemptChannels = [];
    for (const channelId of this.EXEMPT_CHANNELS) {
      const channel = guild.channels.cache.get(channelId);
      if (!channel) {
        missingExemptChannels.push(channelId);
      } else if (!channel.isVoiceBased()) {
        logger.warn('Exempt channel is not a voice channel', { channelId, name: channel.name });
      }
    }

    if (missingExemptChannels.length > 0) {
      logger.warn('Some exempt channels not found in guild', { missingExemptChannels });
    }
